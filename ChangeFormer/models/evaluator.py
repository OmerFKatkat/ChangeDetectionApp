...
class CDEvaluator():
    ...
    def _load_checkpoint(self, checkpoint_name='best_ckpt.pt'):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, checkpoint_name)):
            ...
            _, prefix_action = load_model_state_dict(
                self.net_G, model_state_dict, strict=True)

            self.net_G.to(self.device)
            self.logger.write('Loaded model weights from checkpoint key: %s (%s)\n'
            ...
            if isinstance(checkpoint, dict) and 'best_epoch_id' in checkpoint:
