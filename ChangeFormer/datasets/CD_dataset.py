...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True, to_tensor=True):
        ...
        self.split = split
        # ✅ Fixed: include split subfolder in paths
        self.split_dir = os.path.join(self.root_dir, self.split)
        self.list_path = os.path.join(self.split_dir, LIST_FOLDER_NAME, self.split + '.txt')
    ...
    def __getitem__(self, index):
        ...
        A_path = get_img_path(self.split_dir, self.img_name_list[index % self.A_size])
        B_path = get_img_post_path(self.split_dir, self.img_name_list[index % self.A_size])
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        img   = np.asarray(Image.open(A_path).convert('RGB'))
        img_B = np.asarray(Image.open(B_path).convert('RGB'))
        L_path = get_label_path(self.split_dir, self.img_name_list[index % self.A_size])
